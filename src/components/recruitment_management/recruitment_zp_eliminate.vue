...
<!--              <el-button type="text" size="small" @click="" style="color: #f67183;">删除</el-button>-->
              <el-popconfirm title="是否确认移出?" @confirm="confirmsc(confirmsc(scope.row))" @cancel="cancelsc()">
                <template #reference>
                  <el-button type="text" size="small" style="color: #20ef10;">移出</el-button>
                </template>
              </el-popconfirm>
      ...
      <div class="demo-pagination-block">
        ...
        <el-pagination
            ...
            :pager-count="5"
            background
            @size-change="selecteliminate_plan"
            @current-change="selecteliminate_plan"
        >
        </el-pagination>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    selecteliminate_plan(){
      this.axios
          .get("http://localhost:8010/provider/ResumeVo/ResumePage_D/"+this.pageInfo.currenPage+"/"+this.pageInfo.pagesize)
          .then((response) => {
